...
vec4 PhongLighting(vec3 L, vec3 N, vec3 V, float specPower, vec3 diffuseColor)
{
	...
	return vec4((diffuse*diffuseColor + specular), 1);
}

void main()
{
	vFragColor =  vec4(1.0,0.0,0.0,0.0);
	...
	for (int i = 0; i < MAX_SAMPLES; i++) {
		...
		float sample1 = texture(volume, dataPos).r;			//current sample
		...
		if( (sample1 - isoValue) <= 0  && (sample2 - isoValue) > 0)  {
			...
			//iso-surface
			// isovalue at tc - isoValue should be near zero
			// if (abs(texture(volume, tc).r - isoValue) < 0.001)
			// 	vFragColor =  vec4(0.0,0.0,0.0,1.0);
			// else
				// vFragColor =  PhongLighting(L,N,V, 250, vec3(0.5));
				vFragColor =  PhongLighting(L,N,V,250,  tc);
			break;
		} 
	} 
}